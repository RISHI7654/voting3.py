...
folder_name = "/sdcard/INDRA-RPW-BOOSTING"
...
def logo():
    ...
    print(f"""
{magenta}╭────────────────────────────────────────────────────────────╮
{magenta}│{green}             WELCOME TO INDRU BOOSTING TOOL                {magenta}│
{magenta}╰────────────────────────────────────────────────────────────╯
{magenta}╭────────────────────────────────────────────────────────────╮
...
{magenta}╰────────────────────────────────────────────────────────────╯
{magenta}╭────────────────────────────────────────────────────────────╮
{magenta}│{WHITE}AUTHOR       : INDRA X RISHABH                      {magenta}│
...
{magenta}╰────────────────────────────────────────────────────────────╯
{magenta}╭────────────────────────────────────────────────────────────╮
{magenta}│{WHITE}            AUTO BOOSTING FACEBOOK BY INDRA               {magenta}│
{magenta}╰────────────────────────────────────────────────────────────╯
{magenta}╭────────────────────────────────────────────────────────────╮
...
def overview():
    total_accounts = count_lines("/sdcard/INDRA-RPW-BOOSTING/toka.txt")
    total_pages = count_lines("/sdcard/INDRA-RPW-BOOSTING/tokp.txt")
    total_fras = count_lines("/sdcard/INDRA-RPW-BOOSTING/tokfra.json")
    total_rpws = count_lines("/sdcard/INDRA-RPW-BOOSTING/tokrpw.json")
    print(f'{magenta}╭────────────────────────────────────────────────────────────╮{wh}')
...
def approval():
    try:
        ...
        time.sleep(0.1)
        urls = [
            "https://raw.githubusercontent.com/RISHI7654/approval/refs/heads/main/approval.txt",
            "https://raw.githubusercontent.com/RISHI7654/approval/refs/heads/main/approval.txt",
            "https://raw.githubusercontent.com/RISHI7654/approval/refs/heads/main/approval.txt",
            "https://raw.githubusercontent.com/RISHI7654/approval/refs/heads/main/approval.txt"
        ]        
        ...
        if key_found:
            ...
            os.system(f'am start https://www.messenger.com/t/INDRU.Keith.Grayson.IAmLimitless{tks}')
            time.sleep(1)
            exit() 
...
def cuser(user, passw, user_choice):
    ...
    if "session_key" in pos:
        ...
        if user_choice.lower() in ['n', 'no']:
            with open('/sdcard/INDRU-RPW-BOOSTING/tokpid.txt', 'a') as f:
                f.write(f'{c_user_value}\n')
            with open('/sdcard/INDRU-RPW-BOOSTING/tokp.txt', 'a') as f:
                f.write(f'{pos["access_token"]}\n')
        else:
            with open('/sdcard/INDRU-RPW-BOOSTING/toka.txt', 'a') as f:
                f.write(f'{pos["access_token"]}\n')
            with open('/sdcard/INDRU-RPW-BOOSTING/tokaid.txt', 'a') as f:
                f.write(f'{c_user_value}\n')
        with open('/sdcard/INDRU-RPW-BOOSTING/cok.txt', 'a') as f:
            f.write(f'{cookie}\n')
        with open('/sdcard/INDRU-RPW-BOOSTING/cokid.txt', 'a') as f:
            f.write(f'{c_user_value}\n')
    else:
...
def kaz_rpw(tokens_array):
    data = lod_rpw('/sdcard/INDRU-RPW-BOOSTING/tokrpw.json')
    ...
    seyv_rpw(data, '/sdcard/INDRU-RPW-BOOSTING/tokrpw.json')
...
def kaz(tokens_array):
    data = lod('/sdcard/INDRU-RPW-BOOSTING/toka.json')
    access_tokens = data.get("access_tokens", [])
    for token_info in tokens_array:
        email = token_info["email"]
        access_token = token_info["access_token"]
        ...
        try:
            ...
            is_valid_token = False
    seyv(data, '/sdcard/INDRU-RPW-BOOSTING/toka.json')
...
def rizal():
    try:
        with open('/sdcard/INDRU-RPW-BOOSTING/toka.json', 'r') as file:
            data = json.load(file)
    except FileNotFoundError:
        data = {"access_tokens": []}
    return data
...
def bonifacio(data):
    with open('/sdcard/INDRU-RPW-BOOSTING/toka.json', 'w') as file:
        json.dump(data, file, indent=4)
...
def quickreact():
    ...
    clearr()
    actor_ids = get_ids_tokens('/sdcard/INDRU-RPW-BOOSTING/tokaid.txt')
    tokens = get_ids_tokens('/sdcard/INDRU-RPW-BOOSTING/toka.txt')
...
def auto_comments():
    clearr()
    user_ids = get_ids_tokens('/sdcard/INDRU-RPW-BOOSTING/tokaid.txt')
    access_tokens = get_ids_tokens('/sdcard/INDRU-RPW-BOOSTING/toka.txt')
    current_time = datetime.now()
    user_name = get_user_name()
    post_link = input('ENTER THE FACEBOOK POST LINK : ')
    post_id = linktradio(post_link)
...
def create_page():
        ...
        def delay(dl):
        	...
        	for ti in range(int(dl) , 0, -1):
        	      print(f'\r{GREEN}TASK START > {ti} < INDRU ',end='\r')
        	sleep(0.10)
        for cookies in ck:
        	INDRU = (first)
        	INDRU1 = (middle)
        	INDRU2 = (last)
        	name = str(INDRU+' '+INDRU1+' '+INDRU2)
        	dem = dem+1
...
def perform_reaction_media(url, reaction_type, num_reactions):
    ...
    if not media_id:
        ...
        return
    access_tokens = load_data('/sdcard/INDRU-RPW-BOOSTING/toka.txt')
    reactions_count = 0
...
def auto_group_join(group_id, num_bots):
    access_tokens = get_tokens_from_file('/sdcard/INDRU-RPW-BOOSTING/toka.txt')
    join_count = 0
    failed_count = 0
    for access_token in access_tokens:
...
def acc_checker():
	ids_file_path = '/sdcard/INDRU-RPW-BOOSTING/tokaid.txt'
	tokens_file_path = '/sdcard/INDRU-RPW-BOOSTING/toka.txt'
...
def reset():
    folder_name = "INDRU-RPW-BOOSTING"
    file_names = ["toka.txt", "tokaid.txt", "tokp.txt", "tokpid.txt", "tokfra.json", "tokrpw.json", "cok.txt", "cokid.txt"]
...
def NOTICE():
    os.system('rm -rf /sdcard/.INDRU-BOOSTING')
    os.system('rm -rf .xyz;touch .xyz')
    os.system('rm -rf /sdcard/.CdDc')
    yy = open('/sdcard/.XV', 'r').read()
    clear()
    print(f'{magenta}╭────────────────────────────────────────────────────────────╮')
    print(f'{magenta}│{WHITE}            INDRU BOOSTING TOOL                               {magenta}│')
    print(f'{magenta}│{WHITE}            POGI SI INDRU KEITH GRAYSON💥💚              {magenta}│')
    print(f'{magenta}╰────────────────────────────────────────────────────────────╯')
    print()
